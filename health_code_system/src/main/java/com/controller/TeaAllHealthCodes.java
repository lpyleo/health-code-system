...
@WebServlet(name = "TeaAllHealthCodes", value = "/TeaAllHealthCodes")
public class TeaAllHealthCodes extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if(teachers.getTrole().equals("院级管理员")){
                teachers1=dao.findByFuzzyNameAndCollege(teachers.getTcollege(),teachers.getTcollege());
            }else {
                teachers1=dao.findAllTeachers();
            }
            request.getSession().setAttribute("teachers",teachers);
            request.getSession().setAttribute("teachers1",teachers1);
            RequestDispatcher rd= getServletContext().getRequestDispatcher("/teaAllHealthCodes.jsp");
