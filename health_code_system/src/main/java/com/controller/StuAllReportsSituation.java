...
@WebServlet(name = "StuAllReportsSituation", value = "/StuAllReportsSituation")
public class StuAllReportsSituation extends HttpServlet {
    @Override
        ...
        try {
            ...
            if (dao.stuHasField(today)){
                if(teachers.getTrole().equals("院级管理员")){
                    stuCodes=new ArrayList<StuCodes>(dao.stuCollegeCodes(teachers.getTcollege(),today));
                }else {
                    stuCodes=new ArrayList<StuCodes>(dao.stuAllCodes(today));
                }
            }
